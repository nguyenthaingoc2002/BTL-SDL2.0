...
Mix_Chunk *fire = NULL;
Mix_Chunk *bonus_music = NULL;
Mix_Chunk *game_start_music = NULL;
...
bool LoadMedia ()
{
    ...
    bonus_music = Mix_LoadWAV("sound//bonus.wav");
    game_start_music = Mix_LoadWAV("sound//gamestart.wav");
    game_over_music = Mix_LoadWAV("sound//gameover.wav");
...
int main(  int argc, char* argv[]  )
{
    ...
    Mix_PlayChannel( -1 , game_start_music , 0 );
    TheatsObject tank_enemy1( PlayWindow_X  , PlayWindow_Y , DOWN , 0 , STEP_H , 2 );
    TheatsObject tank_enemy2( PlayWindow_X + PlayWindow_SIZE - MAIN_SIZE  , PlayWindow_Y , LEFT , -STEP_H , 0 , 3 ) ;
    TheatsObject tank_enemy3( PlayWindow_X + PlayWindow_SIZE - MAIN_SIZE  ,PlayWindow_Y + PlayWindow_SIZE - MAIN_SIZE , UP , 0 , -STEP_H , 5 ) ;
    TheatsObject tank_enemy4( PlayWindow_X ,PlayWindow_Y + PlayWindow_SIZE - MAIN_SIZE , RIGHT , STEP_H , 0 , 4 ) ;
    ...
    while ( !is_quit  )
    {
        ...
        while( SDL_PollEvent(&g_event)  )
        {
            ...
            {
                gPause.XuLy(g_screen , g_event , is_quit , can_restart );
            }
        ...
        if ( checkCollision (tank_enemy1.GetRect() , tank.GetRect() ) == true && tank.getLiving() == true  )
        {
            ...
            tank.setLive(-1);
            tank.MAX_NUMBER_BULLET_MAIN = 1;
            if ( tank.getLive() == 0 )
            {
                is_quit = true ;
                can_restart = true;
                timeRestart = SDL_GetTicks();
                if ( back_end_.high_score < score ) back_end_.high_score_update("data//high_score.txt" , score );
                SDL_RenderClear(g_screen);
        ...
        if ( checkCollision ( tank.GetRect() , bonus.GetRect() ) == true && bonus.getLiving() == true )
        {
        ...
        back_end_.TIME( SDL_GetTicks() - timeRestart , gFont , color_ , g_screen );
        back_end_.high_score_render(g_screen);
        SDL_RenderPresent(g_screen);
    ...
    } while ( is_quit == true && can_restart == true );
